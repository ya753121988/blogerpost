...
<head>
    ...
    <style>
        ...
        /* Multi System Buttons */
        .system-toggle { display: flex; gap: 10px; margin-bottom: 20px; }
        .sys-btn { flex: 1; padding: 15px; border: 2px solid #334155; background: #1e293b; color: #fff; border-radius: 10px; font-weight: bold; cursor: pointer; }
...
<div class="container">
    <div class="editor-card">
        ...
        <div class="system-toggle">
            <button id="btn_auto" class="sys-btn active" onclick="switchSystem('auto')">SYSTEM 1: AUTO (TMDB/IMDB)</button>
            <button id="btn_manual" class="sys-btn" onclick="switchSystem('manual')">SYSTEM 2: MANUAL</button>
        </div>

        <div id="auto_search_box">
            <div class="row g-2 mb-4">
                <div class="col-md-5"><input type="text" id="query" class="form-control" placeholder="Search Movie Name..."></div>
                <div class="col-md-3"><input type="text" id="imdb_id" class="form-control" placeholder="IMDB Link or ID (tt123...)"></div>
                <div class="col-md-2">
        ...
        <div id="editor_form" style="display:none;">
            ...
            <div class="section-header">2. CAST MEMBERS <button class="btn btn-sm btn-outline-info float-end" onclick="addManualCast()">+ Add Cast</button></div>
            <div id="e_cast_list" class="row g-3"></div>

            <div class="section-header">3. GALLERY IMAGES <button class="btn btn-sm btn-outline-info float-end" onclick="addManualGallery()">+ Add Gallery</button></div>
            ...
            <!-- Manual System Type Selector -->
            <div class="mt-4"><label class="text-info">Set Link UI Type:</label>
                <select id="manual_type" class="form-select bg-dark text-white" onchange="toggleLinkUI(this.value)">
...
function switchSystem(mode) {
    document.getElementById('btn_auto').classList.toggle('active', mode==='auto');
    document.getElementById('btn_manual').classList.toggle('active', mode==='manual');
    document.getElementById('auto_search_box').style.display = mode==='auto' ? 'block' : 'none';
    if(mode === 'manual') {
...
function toggleLinkUI(type) {
    document.getElementById('movie_ui').style.display = type === 'movie' ? 'block' : 'none';
    document.getElementById('series_ui').style.display = type === 'tv' ? 'block' : 'none';
}
...
async function searchTMDB() {
    ...
    const res = await fetch(`/api/search?q=${q}&imdb=${imdb}&type=${t}`);
...
async function selectItem(id, type) {
    ...
    const trailer = rawTMDB.videos.results.find(v => v.type === 'Trailer');
...
function addSeason(name = "") {
    sCount++; const sId = `s_${sCount}`;
    const div = document.createElement('div'); div.className = 'season-item'; div.id = sId;
    div.innerHTML = `<button class="btn-remove" onclick="this.parentElement.remove()">REMOVE</button>
        <div class="d-flex gap-3 mb-3"><input type="text" class="form-control fw-bold st" value="${name || 'Season '+sCount}">
...
function addEpisode(sId, name = "", links = {}) {
    ...
    let count = parseInt(wrap.dataset.count) + 1; wrap.dataset.count = count;
    const div = document.createElement('div'); div.className = 'episode-item';
    div.innerHTML = `<button class="btn-remove" onclick="this.parentElement.remove()">REMOVE</button>
        <input type="text" class="form-control fw-bold et mb-2" value="${name || 'Episode '+count}">
        <div class="grid-4">
...
async function generateFinalHTML() {
    const castData = [];
    const cEls = document.querySelectorAll('.cn');
    const cIms = document.querySelectorAll('.ci');
    const cIds = document.querySelectorAll('.cid');

    for(let i=0; i<cEls.length; i++){
        let p = { birthday:'Unknown', place_of_birth:'Unknown', biography:'Manual Cast', combined_credits:{cast:[]} };
        if(cIds[i].value !== "0") {
            const res = await fetch(`/api/person?id=${cIds[i].value}`);
            p = await res.json();
        }
        castData.push({
            name: cEls[i].value, img: cIms[i].value, born: p.birthday || 'N/A', place: p.place_of_birth || 'N/A',
            bio: (p.biography || "No Bio").slice(0, 600).replace(/'/g, "").replace(/"/g, "") + "...",
            count: p.combined_credits.cast.length,
            best: p.combined_credits.cast.sort((a,b)=>b.vote_count-a.vote_count).slice(0,5).map(m=>m.title||m.name).join(', ')
        });
    }
    ...
    const data = {
        ...
        type: document.getElementById('manual_type').value,
        cast: castData,
...
function importCode() {
    try {
        ...
        document.getElementById('manual_type').value = meta.type;
...
def search_api():
    q = request.args.get('q'); imdb = request.args.get('imdb'); t = request.args.get('type')
    if imdb:
        im_id = re.search(r'tt\d+', imdb)
        if im_id:
            find_url = f"https://api.themoviedb.org/3/find/{im_id.group()}?api_key={TMDB_API_KEY}&external_source=imdb_id"
            r = requests.get(find_url).json()
            res = r.get('movie_results', []) if t == 'movie' else r.get('tv_results', [])
...
<div class="p-box">
    ...
    <div class="h-ln">CAST MEMBERS</div>
...
<script>
    ...
    function shAc(n,i,b,p,c,w,bio, m_title, m_year) {{
        ...
        document.getElementById('ac-m-title').innerText = m_title;
        document.getElementById('ac-m-year').innerText = "("+m_year+")";
